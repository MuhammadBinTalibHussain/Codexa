...
const Dashboard = () => {
  ...
  const totalReviews = Object.values(reviewsBySnippet).reduce((sum, r) => sum + r.length, 0);
  const avgScore = useMemo(() => {
    const scores = Object.values(reportsBySnippet).filter(Boolean).map((r) => r.overall);
    if (scores.length === 0) return null;
    return Math.round(scores.reduce((a, b) => a + b, 0) / scores.length);
  }, [reportsBySnippet]);

  const recentActivity = useMemo(() => {
    const all = Object.entries(reviewsBySnippet).flatMap(([snippetId, reviews]) =>
  ...
  return (
    <div className="flex">
      ...
      <main className="min-w-0 flex-1 px-4 py-8 sm:px-8">
        <h1 className="mb-6 text-2xl font-bold text-gray-900">Welcome back, {user?.username}</h1>
            ...
            <div className="grid grid-cols-1 gap-6 lg:grid-cols-3">
              <div className="lg:col-span-2">
                <div className="mb-3 flex items-center justify-between">
                  <h2 className="text-lg font-semibold text-gray-900">Your snippets</h2>
                  <select
                    value={sortBy}
                        ...
                        return (
                          <tr key={s._id} className="border-t border-gray-100 hover:bg-gray-50">
                            <td className="px-4 py-2">
                              <Link to={`/snippets/${s._id}`} className="font-medium text-brand-600 hover:underline">
                                {s.title}
                              </Link>
                            </td>
                            <td className="hidden px-4 py-2 text-gray-500 sm:table-cell">{s.language}</td>
                            <td className="px-4 py-2">
                ...
                {totalPages > 1 && (
                  <div className="mt-3 flex items-center justify-center gap-3 text-sm">
                    <button type="button" disabled={page === 1} onClick={() => setPage((p) => p - 1)}
                      className="rounded-md border border-gray-300 px-2 py-1 disabled:opacity-40">Prev</button>
                    <span className="text-gray-500">Page {page} of {totalPages}</span>
                    <button type="button" disabled={page === totalPages} onClick={() => setPage((p) => p + 1)}
                      className="rounded-md border border-gray-300 px-2 py-1 disabled:opacity-40">Next</button>
                  </div>
                )}
              </div>
              ...
              <div>
                ...
                <div className="flex flex-col gap-3">
                  {recentActivity.map((item) => (
                    <div key={item._id} className="rounded-lg border border-gray-200 bg-white p-3 text-sm">
                      ...
                      <p className="mt-1 text-gray-500">{item.comment}</p>
                    </div>
                  ))}
                  {recentActivity.length === 0 && <p className="text-sm text-gray-500">No activity yet.</p>}
                </div>
              </div>
            </div>
          </>
...
const StatusBadge = ({ reviewCount, hasReport }) => {
  if (hasReport) return <span className="rounded-full bg-emerald-100 px-2 py-0.5 text-xs font-medium text-emerald-700">Analyzed</span>;
  if (reviewCount > 0) return <span className="rounded-full bg-amber-100 px-2 py-0.5 text-xs font-medium text-amber-700">Reviewed</span>;
  return <span className="rounded-full bg-gray-100 px-2 py-0.5 text-xs font-medium text-gray-600">Pending</span>;
};
